...
than this library.
This exemple and the library itself is made by #noName 
...
//keep in mind that if you wanna' data going back and forth 
//between loop and isr you will need to declare global variables
...
ISR(TIMER1_COMPA_vect){
  ...
  //to that object ... 
  if(sumo.ultraRead(act_dist)){ //if enemy is in fron of you  
    sumo.setMotors(255, 255);//go at maximum speed to it
  }
  //the function sumo.ir(n) will return true if the 
  //sensor on the n - 1 possition in the sensor 
  //array see anything (it's state is 1) and will
